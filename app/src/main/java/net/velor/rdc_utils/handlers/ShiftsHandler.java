...
public class ShiftsHandler {
    ...
    private static void checkShift(String shiftName) {
        // проверю, есть ли смены в базе данных, если нет- внесу
        DbWork databaseProvider = App.getInstance().getDatabaseProvider();
        Cursor existentShift = databaseProvider.getShiftByShiftName(shiftName);
        if (!existentShift.moveToFirst()) {
            ...
            cv.put(ShiftCursorAdapter.COL_SHIFT_COLOR, ColorsHandler.getRandomColor());
    ...
    public static void checkShift(HashSet<String> shiftsList) {
